...
## 2026-09-09 (A4-d-2 — scope picker, resolved scope, the flattening closed)

- THE FOUR-SHEET FLATTENING IS CLOSED. `runExtraction` now takes a scoped path when a job carries a resolved scope: one sheet, one segment, the chosen period columns. Asserted through the production function (`extractUnderScope`, extracted to its own pure module so the golden test drives the real code path, not a reimplementation of it): every staged line's provenance begins `26-27 Projection!` — no line from `25-26 Actual`, `Dhaulana` or `Upto June Dh` ever appears again.
- Migration 0021: `ingestion_jobs.scope` / `scope_options` / `scope_warnings` / `workbook_fingerprint`, plus the `awaiting_scope` stage between `received` and `approved`. IN-FLIGHT JOBS ARE UNAFFECTED — every column is nullable, every existing row keeps `scope = null`, which the extractor reads as "use the legacy unscoped behaviour". No job moves stage, nothing published is touched, nothing queued can stick in a state the UI cannot clear. A published or rejected job refuses a scope change outright, and `runExtraction`'s existing stage guard (approved/failed only) still prevents reprocessing.
- SCOPE IS ADDRESSED BY (SEGMENT, DATE) AND THE SCHEMA HOLDS THE FULL SET. `scope.periods` is an unbounded array of pairs; A4-d-2's picker offers one segment at a time, but A7-e will write Dhaulana, Greater Noida AND Total into the same array with no migration. Stated in the column comment so nobody reads the UI as the schema's limit.
- AUTO-SELECT, WITH THE RECORD KEPT HONEST. A scope is inferred only where there is exactly one plausible reading at BOTH levels — one plausible sheet, and one segment within it. RPIL's projection sheet reports three units, so it correctly refuses to auto-select. `scope.source` stores `auto` vs `operator`: asked about a figure six months on, "nobody chose this, the system inferred it" and "the operator selected this" are different answers pointing at different places to look. The approve-to-process gate is untouched — auto-selection fills a field, it does not skip a step.
- UNIT BASIS IS ALWAYS VISIBLE AND ALWAYS OVERRIDABLE, not a warning on detection failure. It is the highest-consequence field in scope: every other error makes one line wrong, but rupees-versus-lakhs is a 100,000× error that passes every consistency check we have — the statement still balances, every ratio still reconciles — and only surfaces when a human reads the number. The picker renders the basis with its evidence beside it ("no unit stated — assumed rupees · largest value on this sheet 2,02,00,00,000") at all times. Conversion still happens at exactly one boundary and the multiplier is recorded in the job note.
- WORKBOOK FINGERPRINT stored and deliberately unread. Sheet count, sheet names, per-sheet header shape; two monthly exports of one Tally template fingerprint identically though every figure differs. Nothing consumes it yet — it exists so scope MEMORY is possible later (ADR-016 one level up), because adding the column now is trivial and backfilling it across historical jobs is not.
- Picker (`components/dashboard/review/scope-picker.tsx`) on the existing review route: per sheet, the candidate line count and detected columns, so `26-27 Projection` (66 lines, 18 dated columns, two-sided, 3 units) is distinguishable from `Dhaulana Dep` without opening the file. Unusable sheets are GREYED WITH THEIR REASON, never hidden — an operator hunting a sheet that isn't offered would otherwise assume the tool is broken. Warnings render in the picker, not only in the job note, because choosing is the only moment they can be acted on.
- A scope naming a sheet, segment or date the workbook lacks is REFUSED, never coerced to the nearest match — the same instinct that makes the classifier trustworthy. 16 tests cover it.
- CSVs are deliberately excluded from scoping: one sheet, no name, no tiered header, no segments — scoping offers nothing and would change a path that works today. Single-sheet XLSX still auto-selects, which is the common Tally-export case.
- Footnote reason ordering fixed: the value-gated noise check now runs BEFORE the ratio check, so "5.The reduction in NP margin from 6.50%…" is excluded as a footnote rather than as a "ratio or percentage row". Same outcome, but the reason is what a human reads in A4-e.
- Suite: 10 files / 173 tests, typecheck clean, production build green.
- NOT VERIFIED IN A BROWSER: there is no `.env.local` on this machine, so the staff-gated, DB-backed review route cannot render locally. The picker is verified by typecheck, build and unit tests only — it needs a pass on Preview before it is called done.
...
- NEW `lib/ingestion/workbook.ts` (pure) — the scope model. Introspects a workbook into the choices an operator picks from: per sheet, candidate line count, period columns with canonical dates, two-sided or not, composed multi-tier labels, detected unit basis, segment names. Replaces `sheetsToLines()`'s `flatMap` over every sheet, which is what produced one flat list from four sheets, two plants and three periods.
- PERIODS ARE ADDRESSED BY (SEGMENT, DATE), NOT DATE — caught while running against the real file: `31.03.2026` appears three times per block (Dhaulana, Greater Noida, Total), so a date-only request silently returned the leftmost plant instead of the group total. Segment is first-class in scope, per ADR-014.
- CANONICAL DATE MATCHING WORKS ON THE REAL FILE: the debit header reads `31.03.2026` and the credit header `31.03.026`; both resolve to the same period and both label as `Total · 31.03.2026`.
- NEW `lib/ingestion/row-class.ts` (pure) — every row classified before staging: `line · section_heading · total · subtotal · derived · ratio · note · blank · unclear`. Two rule classes kept deliberately apart. VALUE-GATED (the noise class: signature blocks, report headers, footnotes) may never discard a row carrying a value, and signatory terms match as WHOLE labels — this is the fix for the reference implementation's `director\b` prefix match, which deleted `Director remuneration`, a ₹223.80 L expense. A prose-shaped row that does carry a value becomes `unclear` and is surfaced, never deleted. PATTERN-GATED (totals, subtotals, ratios, derived, headings) hold regardless of value, because those rows almost always carry one.
- STRUCTURE IS NEVER INFERRED FROM ABSENCE OF DATA. Headings need vocabulary AND a structural signal (TO/BY prefix or full capitalisation); capitalisation alone is never sufficient, since this workbook is full of all-caps real lines. Caught by this rule during the build: `BY SALES ACCOUNT` and `Sales` both canonicalise to `SALES`, and promoting on vocabulary alone swallowed the revenue line — the P&L lost its entire top line.
- NEW `lib/ingestion/head-classify.ts` (pure) — the seed SME chart-of-accounts matcher, 11 projection heads, MATCHED BY RULE not by list. Two structural rules outrank keywords: credit-side INDIRECT INCOME reclassifies six cost-reading lines to other income (§5's requirement, implemented structurally), and section context wins inside DIRECT EXPENSES so `Wages & Jobworks` books as production cost. No generic /fee/ or /service/ rule — naming specific fees leaves `Annual Fee`, `Pestseal Service Fee` and `Service Fee for MSME Certification` as honest exceptions instead of confident misclassifications. Closing stock on the credit side carries negative as a contra.
- GOLDEN RESULT: 66 lines, 58 auto-classified, 8 exceptions — the 8 are exactly the fixture's 8. GP reconciles to ₹3,111.17 L / 17.47% and ₹3,579.48 L / 17.72% against the client's stated 3,111.2 / 17.5% and 3,579.5 / 17.7%. FULL-P&L RECONCILIATION PASSES in both years: PBT less the unclassified exceptions equals the workbook's own stated NET PROFIT (₹778.27 L and ₹1,193.88 L).
- `rpil-projection-fixture.v2.json` generated as the corrected golden file (committed under `lib/ingestion/__fixtures__/`; the workbook itself stays out of version control and the end-to-end suite skips without it, so CI runs on synthetic grids). Corrections vs v1: Director remuneration restored; `parent` is now the semantic section for all 66 lines (v1 gave `Bad debt` a parent of `Renewal Fee`); `must_not_stage` replaced by the COMPLETE `excluded` set with a reason per row — v1's list omitted four value-carrying rows that must never stage (`% Total Direct Exp` ₹1,612.79 L, `TO INDIRECT EXPS` ₹2,338.86 L, `NET PROFIT`, `NET PROFIT (%)`); amounts stored in rupees, not lakhs.
- FOUR HONESTY FIXES: (1) `run.ts` no longer prints "validation checks passed" when nothing was checked — zero failures is not zero checks, and the note now distinguishes passed / failed / not-applicable. (2) Migration 0020 splits `mapping_confidence` from `confidence`, with a check constraint making "confidence beside an unmapped line" structurally impossible rather than merely avoided; the review screen renders mapping confidence only. (3) `publishIngestionJob` now READS the job's `validation` and refuses on a failed gate — it never did, so a balance sheet whose assets did not equal its liabilities published on one click. (4) `MAX_SHEETS = 10` replaced by a refusal threshold of 100; row/column bounds now report what they clipped through `warnings`, surfaced in the job note.
- CORRECTION carried from the Stage 0 audit: `MAX_SHEETS` did NOT cause the flattening — all four sheets in the reported provenance are sheets 1, 4, 7 and 8, inside the old cap. `flatMap` alone caused it. The cap still went, for the reason given.
- THREE PRE-EXISTING TESTS UPDATED to the new intended behaviour, each with the reasoning recorded in the test: `Sales A/c` now stores as `Sales` (bookkeeping suffix stripped so one head maps to one memory entry); `Gross Profit` no longer stages (derived — publishing it alongside its components double-counts); mapping memory sets `mappingConfidence` and leaves the amount's own confidence exactly as extraction earned it.
- LEDGER CORRECTED (roadmap): the seed SME chart-of-accounts matcher formally MOVED from A4-e into A4-d-1, and A4-e's scope reduced accordingly — it inherits the matcher as the global seed layer and extends it rather than building it. Process note recorded with it: where a stated acceptance criterion implies work outside the slice, that belongs in the pre-explanation, not the self-review.
- ADR-020 added — derived client figures may be committed; source client documents never are. The RPIL workbook stays out of version control permanently (33 sheets of two units' trial balances, monthly detail and the client's internal observations; git history is permanent and there is no RLS on a repository); tests read it from `RPIL_WORKBOOK` and skip when absent. The derived fixture stays in, at the level already present in migration 0010 and `/r/rpil.html`. Accepted consequence stated in the ADR: CI cannot catch a real-workbook regression, so the golden test must be run locally before any parser change ships.
- ADR-014 addendum: the `(segment, date)` finding is recorded as having come from running against the real file rather than from reasoning — `31.03.2026` appears three times per block, and a date-only request silently returned Dhaulana's column labelled as the company's. A confident wrong number, not an error, and nothing downstream would have caught it. That is the standing argument for keeping a real-workbook test in the loop.
- REFINEMENT NOTED FOR A4-e (not built): derived-line exclusion should be conditional, not absolute. Excluding `Gross Profit` is right when its components are present and staging, wrong when they are absent — a summary P&L that states GP with no cost breakdown has that line as its only source for the figure. Needs a machine-readable reason CODE on `RowClassification` so the review screen can say "excluded because its components staged".
- NOT IN THIS SLICE, by design: the scope picker UI and persisting the chosen scope (A4-d-2), so the live pipeline still flattens sheets — this slice stops the garbage ROWS and the two on-screen contradictions. T-format block splitting, unit conversion and the memory extension are exercised by the new modules but not yet wired into the staging path.

## 2026-09-09 (Stage 0 closed — A4-d / A4-e / A7 approved; fixture verified)

- STAGE 0 AUDIT delivered against the A7 brief's §5–§9: `docs/Stage 0 — Extraction Rebuild and Projection Engine.md`. Verdict — the four defects Parth saw on the RPIL review screen trace to four specific causes, three of them cheap: the "validation checks passed" summary is one ternary in `run.ts` (zero *failures* is not zero *checks*; the gates themselves were already honest); "100% confidence" beside "— not mapped —" is `confidence: 1` hardcoded in the parser, one field carrying two meanings; footnotes/totals stage because nothing filters a row (`isTotalLabel()` exists but is used only by validation). The fourth — four sheets flattened into one list — is `sheetsToLines()` `flatMap`ping every sheet, and needs the scope model.
- BLOCKING GAP FOUND, not named in the brief: there is no line-level financial store. `kpi_values` is KPI-level over a ~20-key catalogue; `extracted_lines` is staging (mutable, staff-only, soft-deleted on re-extract, addressed by job not period). And `lib/review/actions.ts` *rejects* a publish where two lines map to one KPI. §8's line-level projection was impossible until this is fixed. Became A7-a, and moved AHEAD of A4-e so the publish path is written once.
- SIX repo-vs-prompt disagreements raised and resolved by Parth: `Project State.md` does not exist (Changelog IS the state doc — extend it, don't recreate it); migrations run to 0019 not 0013; A4-c already shipped so the new work is A4-d/A4-e; the A7–A9 master prompt was never committed (three-statement spec restated in the brief instead); **A10 = Bank Finance Pack (CMA)** — Cost of Project, term loan schedule, MPBF Tandon Method II, DSCR, mandatory non-bypassable analyst sign-off, a separate deliverable not a dashboard tab; DoD line count corrected to 65/57/8.
- LEGAL-ENTITY QUESTION SETTLED (ADR-019): migration 0009 was right and the brief's "two legal units" was loose language. Dhaulana and Greater Noida are two plants of ONE company — the workbook carries a single company-headed P&L with Dhaulana / Greater Noida / Total columns side by side. Consolidation is summation, not elimination. Revenue/direct/indirect project per segment; **tax is computed once at entity level**; depreciation and finance cost follow their schedules per segment where attributable; the balance sheet is entity-level only — segment balance sheets are not maintained and must not be invented.
- ADRs 011–019 appended: six-stage pipeline, universal eight-basis set, seasonality-adjusted annualisation with a plausibility guard, per-segment projection, mandatory provenance, operator-works-by-exception, label-only memory keying with a global SME seed layer, line-level store with KPI as derived roll-up, and the entity-vs-segment rule.
- AMENDMENTS A4-d / A4-e / A7 (a–e) appended with A8/A9/A10 dependencies. Approved order: **A4-d → A7-a → A4-e → A7-b → A7-c → A7-d → A7-e → A8 → A9**.
- GOLDEN FIXTURE RECEIVED and independently verified: `rpil-projection-fixture.json` is internally exact — all 20 reconciliation figures across both years derive from its 65 lines to the paisa (GP ₹3,111.17 L / 17.47% and ₹3,579.48 L / 17.72%, against the client's stated 3,111.2 / 17.5% and 3,579.5 / 17.7%). Workbook cross-checked: 33 sheets confirmed, `26-27 Projection` is sheet 4, T-format with label columns A and L confirmed, two header tiers confirmed (row 5 unit / row 6 date), the hand-typed date disagreement is real (`J6 = 31.03.2026` vs `U6/O6/R6 = 31.03.026`), `#REF!` present, and the section-heading-carrying-a-subtotal case is real (`TO INDIRECT EXPS` = 2338.86).
- THREE FIXTURE FINDINGS raised to Parth before building (see the response of the same date): `must_not_stage` is illustrative not exhaustive (`% Total Direct Exp`, `TO INDIRECT EXPS`, `NET PROFIT`, `NET PROFIT (%)` also carry values and must not stage); the `parent` field is unreliable as a semantic section (four zero-valued real expense heads — GST Expense, ESIC Employee's Cont., Renewal Fee, Plot Allotment Expense — were read as headings); and **`Director remuneration` ₹223.80 L in both years is missing from the fixture's 65 lines** — proved arithmetically (fixture indirect + depreciation + finance + the 8 unmatched + 223.80 = the workbook's own `TO INDIRECT EXPS`, exact in both years) and formatting-checked (row 66 is styled identically to its staged neighbours, so no rule justifies the omission).
- CORRECTION to the Stage 0 audit: `MAX_SHEETS = 10` did NOT cause the flattening Parth saw — all four sheets in his provenance (`25-26 Actual`, `26-27 Projection`, `Dhaulana`, `Upto June Dh`) are sheets 1, 4, 7 and 8. The cap still goes (23 sheets were silently discarded), but the cause was `flatMap`, not truncation.
- No code written. A4-d-1 pre-explained and awaiting approval.
